        ...
        return (
            <div className={cn("border border-black bg-white p-6", className)}>
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="text-neutral-500 text-sm">No store data</p>
                    <p className="text-neutral-400 text-xs mt-1">Upload receipts to see analysis</p>
                </div>
            </div>
        )
    }
        ...
        <div className={cn("border border-black bg-white", className)}>
            ...
            <div className="p-6">
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2 min-w-0 flex-1">
                                        <span className="font-bold text-sm w-5 text-neutral-400 flex-shrink-0">
                                            #{index + 1}
                                        </span>
                                    ...
                                    <div className="flex items-center gap-4 flex-shrink-0">
                                        ...
                                        <span className="font-bold font-mono text-sm min-w-[80px] text-right">
                                            ${store.totalAmount.toFixed(2)}
                                        </span>
                                    </div>
                                </div>
                                
                                <div className="relative h-6 bg-neutral-100 border border-black/10">
                                    <motion.div
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-neutral-200">
                                <th className="text-left py-2 font-bold text-xs uppercase tracking-wider text-neutral-500">Store</th>
                                <th className="text-right py-2 font-bold text-xs uppercase tracking-wider text-neutral-500">Avg/Visit</th>
                                <th className="text-right py-2 font-bold text-xs uppercase tracking-wider text-neutral-500">% Share</th>
                            </tr>
                        </thead>
                        <tbody>
                            {storeData.map((store, index) => (
                ...
                <div className="mt-4 pt-4 border-t border-black flex justify-between items-center">
                    <span className="font-bold uppercase text-xs tracking-wider">Total (Top 5)</span>
                    <span className="font-bold font-mono text-lg">${totalSpend.toFixed(2)}</span>
                </div>
            </div>
        </div>
    )
