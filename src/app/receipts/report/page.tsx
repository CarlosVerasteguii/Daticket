...
function ReportContent() {
    ...
    const handleEmailShare = () => {
        const subject = encodeURIComponent(`Expense Report - ${reportPeriod}`)
        const body = encodeURIComponent(
            `Expense Report Summary\n\n` +
            `Period: ${reportPeriod}\n` +
            `Total Spent: $${totalAmount.toFixed(2)}\n` +
            `Number of Receipts: ${receipts.length}\n` +
            `Average: $${receipts.length > 0 ? (totalAmount / receipts.length).toFixed(2) : '0.00'}\n\n` +
            `Category Breakdown:\n` +
            categoryBreakdown.map(([cat, data]) => 
                `• ${cat}: $${data.total.toFixed(2)} (${data.count} receipts)`
            ).join('\n') +
            `\n\n---\nGenerated by Daticket`
        )
        ...
        if (navigator.share) {
            try {
                await navigator.share({
                    title: `Expense Report - ${reportPeriod}`,
                    text: `Total: $${totalAmount.toFixed(2)} | ${receipts.length} receipts`,
